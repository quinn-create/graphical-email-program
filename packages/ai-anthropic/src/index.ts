...
export interface AiAnthropicConfig {
  apiKey?: string;
  baseUrl?: string;
 ...
 */
export function createAiAnthropicAdapter(config: AiAnthropicConfig = {}): AiProviderAdapter {
  const useMock = config.mock !== false && !config.apiKey;
  if (useMock) {
    return createMockAdapter("ANTHROPIC" as ProviderType, defaultModelsFor("ANTHROPIC" as ProviderType));
  }
  ...
  return createHttpAiAnthropicAdapter(config);
}

function createHttpAiAnthropicAdapter(config: AiAnthropicConfig): AiProviderAdapter {
  const mock = createMockAdapter("ANTHROPIC" as ProviderType, defaultModelsFor("ANTHROPIC" as ProviderType));
  return {
    ...mock,
    async testConnection() {
      if (!config.apiKey && "ANTHROPIC" !== "OLLAMA") {
        return { ok: false, message: "API key required for live ANTHROPIC connection" };
      }
      ...
      return {
        ok: false,
        message: "Live ANTHROPIC adapter configured — connection not verified (no network test in scaffold). Use Mock mode or run connection test from Settings with credentials.",
      };
    },
  };
}

export const PROVIDER_TYPE = "ANTHROPIC" as const;
