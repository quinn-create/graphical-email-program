 ...
 */
export function createAiXaiAdapter(config: AiXaiConfig = {}): AiProviderAdapter {
  const useMock = config.mock !== false && !config.apiKey;
  if (useMock) {
    return createMockAdapter("XAI" as ProviderType, defaultModelsFor("XAI" as ProviderType));
  }
  ...
  return createHttpAiXaiAdapter(config);
}

function createHttpAiXaiAdapter(config: AiXaiConfig): AiProviderAdapter {
  const mock = createMockAdapter("XAI" as ProviderType, defaultModelsFor("XAI" as ProviderType));
  return {
    ...mock,
    async testConnection() {
      if (!config.apiKey && "XAI" !== "OLLAMA") {
        return { ok: false, message: "API key required for live XAI connection" };
      }
      ...
      return {
        ok: false,
        message: "Live XAI adapter configured — connection not verified (no network test in scaffold). Use Mock mode or run connection test from Settings with credentials.",
      };
    },
  };
}

export const PROVIDER_TYPE = "XAI" as const;
